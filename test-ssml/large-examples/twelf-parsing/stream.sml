...
sig
  ...
  (*[ sortdef 'a infStream < stream
      datasort 'a infFront = Cons of 'a * 'a infStream       ]*)
...
struct
  ...
  fun expose (Stream (d)) = d ()
  fun 'a delay (d) =
      let (*[ memo : (*(unit -> 'a front) ref,*) (unit -> 'a infFront) ref ]*)
          val memo  = ref (fn () => raise Uninitialized)
          fun memoFun () =
              let
                  val r = d ()
               in
      ...
      in
        memo := memoFun ; (Stream (*[ :> (unit -> 'a infFront) -> 'a infStream ]*))
                              (fn () => !memo () (*[ :> unit -> 'a infFront ]*))
      end
