    ...
    fun rewriteDatBind(datbind, typbind) =
      ...
      let
        ...
        fun replaceTy tyvarseq tyseq ty = 
          ...
          | INTERty(i, ty1, ty2) => 
              INTERty(i, replaceTy tyvarseq tyseq ty1,
                      replaceTy tyvarseq tyseq ty2)
        ...
        fun rewriteTy ty =
          ...
          | INTERty(i, ty1, ty2) => 
              INTERty(i, rewriteTy ty1, rewriteTy ty2)
          | PARty(i, ty) =>
    ...
    fun layeredPat (i, idPat, asPat) =
	  ...
	  in
		...
		LAYEREDpat (i, OP_OPT (longIdToId id, withOp), SOME ty, asPat)
              (* The info isn't quite right in the following *)
	    | SORTEDpat (_, ATPATpat (_, LONGIDatpat (_, OP_OPT (id, withOp))), ty) =>
		LAYEREDpat (i, OP_OPT (longIdToId id, withOp), NONE, SORTEDpat(i, asPat, ty))
