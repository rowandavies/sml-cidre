        ...
        fun try_simple_fvalbindToValbind(info_fvalbind,id,rhsList: (atpat list * exp * ty option) list) = 
          let
            ...
            and certainMatch_pat(ATPATpat(_,atpat)) = certainMatch_atpat atpat
              ...
              | certainMatch_pat(SORTEDpat(_,pat,ty)) = certainMatch_pat pat
    ...
    and resolveDec(iBas, dec): Basis * dec =
      case dec
        of VALdec(i, tyvars, valbind) =>
         ...
         | VALsdec(i, valsdesc) => (emptyB, VALsdec(i, valsdesc)) (* types only *)
         ...
         | DATASORTdec(i, datbind) =>
             (emptyB, DATASORTdec(i, resolveDatbind(iBas, datbind)))
    ...
    and resolveExp(iBas, exp) =
      case exp
        of ATEXPexp(i, atexp) =>
         ...
         | SORTEDexp(i, exp, ty) =>
             SORTEDexp(i, resolveExp(iBas, exp), ty)
    ...
    and resolvePat(iBas, pat) =
      case pat
        of ATPATpat(i, atpat) =>
         ...
         | SORTEDpat(i, pat, ty) =>
             SORTEDpat(i, resolvePat(iBas, pat), ty)
