...
(* Modified to work with RefInfo as well as ElabInfo - Rowan *)
...
(*     structure TypeInfo = ElabInfo.TypeInfo    (not needed?)  *)
   ...
   (* Yup, side-effect time; this is the best way to take note when an
    ...
    val report_SourceInfo_in_ElabInfo = ElabInfo.report_SourceInfo
    ...
    fun check i =
      if report_more_errors() then
		...
		Report.line ""
		// (if ElabInfo.ErrorInfo.errorIncludesSourceInfo ei
                      then Report.null
                      else report_SourceInfo_in_ElabInfo i)
    ...
    fun list_walk walk [] = ok
      | list_walk walk (h::t) = walk h // list_walk walk t
   ...
   (* MODULES: *)
	...
	of VALspec(i, valdesc) =>       check i // walk_Valdesc valdesc
	 | VALsspec(i, valdesc) =>       check i // walk_Valdesc valdesc
	 | TYPEspec(i, typdesc) =>      check i // walk_Typdesc typdesc
	 | SORTspec(i, sortdesc) =>      check i // walk_Sortdesc sortdesc
	 | SUBSORTspec(i, spec, longids1, longids2) =>
             check i // walk_Spec spec // walk_IdInfoList longids1 // walk_IdInfoList longids2
	 ...
	 | DATATYPE_REPLICATIONspec(i, tycon, longtycon) =>
	     check i
	 | DATASORTspec(i, datdesc) =>  check i // walk_Datdesc datdesc
    ...
    and walk_Sortdesc sortdesc =
      case sortdesc
	of SORTDESC(i, _, _, _, _, sortdesc_opt) =>
	  check i // (*walk_SubSortDesc subsortdesc //*) walk_opt walk_Sortdesc sortdesc_opt

(*    and walk_SubSortDesc subsortdesc =
        case subsortdesc
          of SUBSORTDESC(i, longids1, longids2) =>
           check i // walk_IdInfoList longids1 // walk_IdInfoList longids2   *)
   ...
   (* CORE: *)
	...
	of VALdec(i, tyvars, valbind) =>
         ...
         | VALsdec(i, valsdesc) => check i // walk_Valsdesc valsdesc
    ...
    and walk_Valsdesc (VALSDESC (i, _, tys, valsdesc_opt)) =
      check i // list_walk walk_Ty tys // walk_opt walk_Valsdesc valsdesc_opt
	...
	of ATPATpat(i, atpat) =>
	 ...
	 | SORTEDpat(i, pat, tys) =>
	     check i // walk_Pat pat // list_walk walk_Ty tys
	...
	of ATEXPexp(i, atexp) =>
	 ...
	 | SORTEDexp(i, exp, tys) =>
	     check i // walk_Exp exp // list_walk walk_Ty tys
	...
	of TYVARty(i, _) =>
	 ...
	 | CONty(i, tys, _) =>
	     check i // Edlib.List.foldR (fn a => fn b => walk_Ty a // b) ok tys
