...
struct
  ...
  fun copy (a, n) = (* h *) let val a2 = Array.array (n, Nil)
				val () = Array.copy {src=a, dst=a2, di=0, si=0, len=SOME n}
			    in
				(a2, n)
			    end
(*          (Array.tabulate (n, fn i => Array.sub (a, i)), n) *)

  (* Persistant hash tables are slow to build... - Rowan *)
  fun insertPersistant h e = (let val h2 = copy h 
			      in 
				  insert h2 e; 
				  h2
			      end)
