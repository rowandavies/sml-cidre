...
functor ElabTopdec
  (structure PrettyPrint : PRETTYPRINT
   ...
   structure RefDec : REFDEC where type Comp.Error = ElabInfo.ErrorInfo.RefineErrorInfo.Error
   ...
   structure Report : REPORT
     sharing type Report.Report = ElabInfo.ErrorInfo.RefineErrorInfo.Report = Flags.Report     
   ...
   sharing type OG.longid = IG.DecGrammar.Ident.longid   
   sharing type OG.DecGrammar.longid = IG.DecGrammar.Ident.longid
(*   sharing type OG.tyvar = StatObject.ExplicitTyVar
   sharing type OG.DecGrammar.tyvar = StatObject.ExplicitTyVar *)
   sharing type OG.DecGrammar.TyVar.SyntaxTyVar = StatObject.ExplicitTyVar (* For SML/NJ 110.0.3 *)
   ...
   sharing type SortName.sortcon = IG.tycon
   sharing type SortName.Variance = IG.DecGrammar.TyVar.Variance
   ...
   sharing type ModuleEnvironments.strid = IG.strid = ModuleEnvironments.rEnv.strid
   ...
   sharing type ModuleEnvironments.rEnv.StringTree = PrettyPrint.StringTree
   sharing type ModuleEnvironments.rEnv.Env = ModuleStatObject.rEnv
   sharing type ModuleEnvironments.rEnv.TyNameEnv = ModuleStatObject.rT
   sharing type ModuleEnvironments.rEnv.ExplicitTyVar = StatObject.ExplicitTyVar
   sharing type ModuleEnvironments.rEnv.id = IG.id
   sharing type ModuleEnvironments.rEnv.SortName = SortName.SortName
   sharing type ModuleEnvironments.rEnv.TyName = StatObject.TyName.TyName
   sharing type ModuleEnvironments.rEnv.tycon = IG.tycon
   sharing type ModuleEnvironments.rEnv.longtycon = IG.longtycon
   sharing type ModuleEnvironments.rEnv.SortFcn = RefObject.SortFcn
   sharing type ModuleEnvironments.rEnv.Sort = RefObject.Sort
   sharing type ModuleEnvironments.rEnv.Variance = SortName.Variance
   ...
   sharing type ElabDec.PreElabDatBind = IG.DecGrammar.datbind
   sharing type ElabDec.PostElabDatBind = OG.DecGrammar.datbind
   sharing type ElabDec.Env = Environments.Env
   sharing type ElabDec.TyEnv = Environments.TyEnv
   ...
   sharing type RefDec.Context = ModuleEnvironments.rContext
   sharing type RefDec.TyNameEnv = ModuleEnvironments.rEnv.TyNameEnv
   sharing type RefDec.Env = ModuleEnvironments.rEnv
   sharing type RefDec.PostElabDec = OG.dec
   sharing type RefDec.PostElabTy = OG.ty
   sharing type RefDec.PostElabDatBind = OG.DecGrammar.datbind
   sharing type RefDec.Context = ModuleEnvironments.rContext
   (* sharing type RefDec.Comp.Error = ElabInfo.ErrorInfo.RefineErrorInfo.Error *)

   sharing type RefObject.Type = StatObject.Type
   sharing type RefObject.Sort = RefDec.Sort
   sharing type RefObject.SortScheme = ModuleEnvironments.rEnv.SortScheme
   ...
   sharing type RefObject.SortVar = ModuleEnvironments.rEnv.SortVar
