...
*)
    ...
    fun info_on_strexp (STRUCTstrexp (info, strdec)) = info
      | info_on_strexp (ASSUMEstrexp (info, sigexp)) = info
    ...
    fun info_on_spec (VALspec (info, valdesc)) = info
      | info_on_spec (VALsspec (info, valdesc)) = info
      | info_on_spec (TYPEspec (info, typdesc)) = info
      | info_on_spec (SORTspec (info, sortdesc)) = info
      | info_on_spec (SUBSORTspec (info, _, _, _)) = info
      ...
      | info_on_spec (DATASORTspec (info, datdesc)) = info
    ...
    fun info_on_sortdesc (SORTDESC (info, tyvars, tycon, _, longtycon, srtdesc_opt)) = info

    fun info_on_datdesc (DATDESC (info, tyvars, tycon, condesc, datdesc_opt)) = info
      
    fun info_on_condesc (CONDESC (info, longid, ty_opt, condesc_opt)) = info
	...
	of STRUCTstrexp (info, strdec) => STRUCTstrexp(f info, map_strdec_info f strdec)
	 | ASSUMEstrexp (info, sigexp) => ASSUMEstrexp(f info, sigexp)
	...
	of STRUCTstrexp(_, strdec) =>
	 ...
	 | ASSUMEstrexp(_, sigexp) =>
	     NODE{start="struct (*[ assume ", finish=" ]*) ... end", indent=INDENT,
		     children=[layoutSigexp sigexp], childsep=NOSEP
		    }
	...
	of VALspec(_, valdesc) =>
	 ...
	 | SORTspec(_, sortdesc) =>
	     NODE{start="sort ", finish="", indent=INDENT,
		     children=[layoutSortdesc sortdesc], childsep=NOSEP
		    }

	 | SUBSORTspec(_, spec, longtycons1, longtycons2) =>
            let fun layout_tycons [] =
                       NODE{start="", finish="", indent=0,children=[], childsep=NOSEP}
                   | layout_tycons longtycons = 
                       NODE{start="", finish="", indent=0,
                            children=map (LEAF o TyCon.pr_LongTyCon o strip_info) longtycons,
                            childsep=LEFT " "
                           }
                 val subspec =  NODE{start=" subsort", finish="", indent=0,
                                     children=[layout_tycons longtycons1,
                                               layout_tycons longtycons2],
                                     childsep=LEFT "<"
                                    }
             in
               layout_together [layoutSpec spec, subspec] INDENT
             end
	 ...
	 | DATASORTspec(_, datdesc) =>
	     NODE{start="datasort ", finish="", indent=INDENT,
		     children=[layoutDatdesc datdesc], childsep=NOSEP
		    }
    ...
    and layoutSortdesc sortdesc =
      let
	val sortdescs = makeList (fn SORTDESC(_, _, _,  _, _, opt) => opt) sortdesc

        fun pr_IsSub false = "|:"
          | pr_IsSub true = "<"
	fun layout1(SORTDESC(_, tyvarseq, tycon, isSub, longtycon, _)) =
	  NODE{start="", finish="", indent=0,
		  children=(case DecGrammar.layoutTyvarseq tyvarseq
			      of SOME t => [t]
			       | NONE => nil
			   ) @ [layoutAtom TyCon.pr_TyCon tycon,
                                layoutAtom pr_IsSub isSub,
                                LEAF (TyCon.pr_LongTyCon longtycon) ],
		  childsep=RIGHT " "
		 }
      in
	NODE{start="", finish="", indent=0,
		children=map layout1 sortdescs,
		childsep=LEFT " and "
	       }
      end

(*    and layoutSubSortdesc (SUBSORTDESC (_, spec, longtycons1, longtycons2)) =
        let fun layout_tycons header [] =
  	          NODE{start="", finish="", indent=0,children=[], childsep=NOSEP}
              | layout_tycons header longtycons = 
   	          NODE{start=header, finish="", indent=0,
		       children=map (LEAF o TyCon.pr_LongTyCon o strip_info) longtycons,
  		       childsep=LEFT " "
	              }
            val subspec =  NODE{start=" subsort", finish="", indent=0,
        ...
        in
          layout_together [layoutSpec spec, subspec]
        end
*)
	...
	fun layoutCondesc condesc =
	  let
	    ...
	    fun layout1(CONDESC(_, longid, ty_opt, _)) =
	      NODE{start="", finish="", indent=0,
		      children=
		        layoutAtom Ident.pr_longid longid
