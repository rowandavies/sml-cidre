...
functor RefinedEnvironments
  ...
  struct
	...
	(* !debug_indent < 10 andalso *) (* !debug_subsortname andalso*)
				   ...
				   if now - (!last_report_time) > 1000 then
	...
	let val print_now1 = debug_print_is_on()
            ...
            val _ = debug_push_maybe print_now strs
	in
            ()
	end
...
(*    fun eq_sn2 ((sn11, sn12), (sn21, sn22)) = SortName.eq (sn11, sn21) andalso
    ...
    fun hashWords (x, y) = (x mod 777337) + ((y mod 444337) * 1111) + 4443337
...
(*       and Str = STR of {m: StrName, E: Env} *)
    ...
    fun layoutRL (REFLATTICE m) = PP.LEAF "<REFINEMENT LATTICE OMITTED>"
      ...
      let val Rdom1 = Rdom R1
          ...
          fun match_sortname (sn1, sn2) =
              let val cnjs1 = SortName.conjuncts sn1
                  val cnjs2 = SortName.conjuncts sn2  (* HACK.  Should almost always work. *)
...
(*                    val _ = print (RO.pr_Sort srtcons) *)
		  ...
		  end handle _ => (print "WARNING: Lookup_longid\n"; 
				   SOME (LONGCON ssch)))  (* HACK, shouldn't happen. - Rowan *)
                | result => result)
    ...
    ********)
     ...
     fun subSortName0 _ ([], _) = Crash.impossible "RefinedEnvironments.subSortName0(1)"
       ...
       let val orig_indent = !debug_indent
           val _ = debug_push (fn ()=> ["subSName0TOP: " ^ (pr_SortNames sortnames1) ^ " <| " ^
...
*)
	 ...
	 val result_opt = 
	     case HashTable.find memoSSN sns1_snsl2'
              of SOME (b,[]) => (out_debug (fn ()=>"MEMO"); SOME (b, assumptions) )(* memoized *)
               | NONE => NONE
       in
...
*)
		...
		val srts2 = map (fn NONE => Crash.impossible "RefinedEnvironments.subSortopt"
			 	  | SOME sfcn' => RO.applySortFcn (sfcn', sortsvs))
			        sfcn_opts
		val _ = debug_push (fn () => "subSortOpt:START" :: "   " ^ pr_sort sort :: pr_sorts srts2)
...
*)
	   ...
	   case List.find matches_goal assumptions of
         ...
         val _ = if (memo_success orelse (not res)) then
          ...
          val _ = (* if  len_assume >= !reported_size_assumptions then *)
                  if (* !debug_indent < 5 *) true then
		      ((*out_debug_ssn (fn () => ("subSortName0 END: length r_assumptions = " ^ Int.toString len_assume ^ ""));*)
(*                       pr_assumptions "IN ASSUMPTIONS:" assumptions;
                         pr_assumptions "OUT_ASSUMPTIONS:" r_assumptions; *)
                     debug_pop (fn () => [("subSortName0: " ^ (pr_ssns (sortnames1', sortnames_list2')) ^
   				           " RESULT: " ^ (Bool.toString res))])
                       (* reported_size_assumptions := len_assume *))
          
                  else  ()

          (* The following line shouldn't be need! *)
...
*)
    ...
    fun on_RL conjSN phi (REFLATTICE m) =
          let  val onSN = on_SortName' conjSN phi
