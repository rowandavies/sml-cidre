...
*)
    ...
    fun layoutR (SORTNAMEENV m) =  (* USED BY TopLevelReport! *)
      let val svnames = ref (RO.newSVNames ())   (* FIX:   RefObject.TVNamesOfSVNames SVnames *)
       ...
       case conjoinRL' RL (sn1, sn2)
         of SOME sortname => sortname
          | NONE => if !(Flags.lookup_flag_entry "elaboration_error_found") then  sn1 (* Elab err *)
                          ...
                          print ("BUG: RefinedEnvironments.conjoinRL " ^ 
                                      (SortName.pr_SortName ("", sn1)) ^ ", " ^
                                      (SortName.pr_SortName ("", sn2)) ^ 
                                      " RL = " ^ (prRL RL) ^ "\n")
                ...
                of SOME (TYSTR {RL, ...}) => conjoinRL RL (sn1, sn2)
                 | NONE => 
                   if !(Flags.lookup_flag_entry "elaboration_error_found") then  sn1 (* Elab err *)
                          ...
                          print ("BUG: REnv.conjoinSortNameT.  Type name missing: (" ^ 
                                         (SortName.pr_SortName ("", sn1)) ^ ", " ^
                                         (SortName.pr_SortName ("", sn2)) ^ ")\n" ) )
...
(*record*) | NONE => case (SO.Type.to_RecType ty) of (SOME rectype) =>
    ...
    ********)
     ...
     fun subSortName0 _ ([], _) = Crash.impossible "RefinedEnvironments.subSortName0(1)"
       ...
       let val orig_indent = !debug_indent
           val result = 
       (out_debug(fn ()=> "subSortName0: " ^ (pr_SortNames sortnames1) ^ " < " ^
        ...
        case sortnames_list2 of [sortnames2 as _::_::_] => 
               foldl (fn (sn2, (false, _)) => (false, [])
                       | (sn2, (true, r_assumptions)) =>
                                      let val (res, r_assumptions2) = 
					      subSortName0 (T, memo_success, assumptions) 
							   (sortnames1, [[sn2]])
				      in (res, unionSN (r_assumptions, r_assumptions2))
				      end )
		     (true, [])
                     sortnames2
        | _ =>  (* otherwise *)
       ...
       let
         (* Possible additional optimizations to consider:
             - Attempt to lookup in lattice, particularly if only one disjunct on RHS.
             - Return successful subgoals, along with the assumptions they depend on.  
               Or memoize the subgoals with the assumptions.
             - Optimizations based on Hosoya, Vouillon, Pierce
         *)
         ...
         fun subset ([], _) = true
           | subset (_, []) = false
           | subset (h1::t1, h2::t2) = 
             SortName.leq (h2, h1) andalso
             (if SortName.eq (h1, h2) then subset(t1, t2) else subset(h1::t1, t2))

         (* Match a goal against the assumptions.  *)
         (*[ val matches :> sortname list * sortname list list -> bool ]*)
         fun matches (sortnames11, sortnames_list22) (sortnames : SortName list, sortnames_list : SortName list list) =
	 ...
	 val result_opt = 
	     case HashTable.find memoSSN sns1_snsl2'
              of SOME (b,[]) => (out_debug (fn ()=>"MEMO"); SOME (b, []) )(* return the memoized value *)
               | SOME (true, r_assumptions) => 
                   foldl (fn (_, NONE) => NONE
                           | (r_assumption1, SOME (true, r_assumptions2)) =>
			       case List.find (fn assumption2 => matches r_assumption1 assumption2)
					      assumptions
				of SOME assumption2 =>
				   SOME (true, unionSN (r_assumptions2, [assumption2]) )
	                         | NONE => NONE )
                         (SOME (true, []))
			 r_assumptions
               | NONE => NONE
       ...
       let  
         ...
         (*[ val expC :> con * SortName list * SortFcn option list -> SortFcn option list ]*)
         ...
         fun subSortopt (NONE, []) = (false, [])  (* No argument to constructor, RHS is empty. *)
           | subSortopt (NONE, _) = (true, [])    (* No argument, RHS non-empty. *)
           | subSortopt (SOME sfcn, sfcn_opts) =
            let val (svs, sort) = RO.applySortFcnVars sfcn
                ...
                val r_assumptions = ref []
                fun subSN sns_arg = (* Accumulate assumptions using a reference.  *)
		    let val (res, r_assumptions1) = 
			    subSortName0 (T, memo_success, new_assumption::assumptions)
					 sns_arg
			val _ = r_assumptions := unionSN (!r_assumptions, r_assumptions1)
		    in
			res
              	    end
		val result = 
		    RO.subSortList (subSN, conjSN) (* Check subsorting, using subSN.  *)
				   (sort, (map (fn NONE => Crash.impossible 
							       "RefinedEnvironments.subSortopt"
						 | SOME sfcn' => RO.applySortFcn (sfcn', sortsvs))
					       sfcn_opts) )
		val return_assumptions = List.filter (fn x => not (eqSSN (x, new_assumption)))
						     (!r_assumptions)
            in
         ...
         val (res, r_assumptions) =   (* First case is an optimization *)
           if List.exists (fn sns => subset (sns, sortnames1')) sortnames_list2' then
              (true, [])  (* no assumptions needed *)
           else 
	   case List.find matches_goal assumptions of
	       (SOME assumption) => (true, [assumption])
	     | NONE => 
           foldl    (* Expand for each constructor to get sub-goals.  *)
             (fn (con, (false, _)) => (false, [])
               | (con, (true, r_assumptions)) =>
               let val sortoptRHS =
                 foldl (fn (x,r) => (expandConj (con, x)) @ r) [] 
                       sortnames_list2' 
               in
                 foldl (fn (sortoptLHS, (false, _)) => (false, [])
                         | (sortoptLHS, (true, r_assumptions2)) => 
					    let val (res3, r_assumptions3) = 
						    subSortopt (sortoptLHS, sortoptRHS)
					    in  (res3, unionSN (r_assumptions, r_assumptions3))
		                            end 
                       )
                       (true, r_assumptions)
                       (expandConj (con, sortnames1'))
               end)
             (true, [])  (* Result and assumptions *)
             (domCE CE)
         val _ = if (memo_success orelse (not res)) then
                   ...
                   HashTable.insert memoSSN (sns1_snsl2', (res, r_assumptions))
    ...
    fun completeT (C : Context, TRs : (TyName * SortNameEnv) list, RE : SortEnv) 
      ...
      let
        ...
        val _ = pr_debug "completeT(end): (T, VE) = " 
                         (fn () => PP.layout_pair layoutT layoutVE (T_withCE, changedVE))
    ...
    structure Rea = struct  (* See Environments.Realisation *)
      ...
      fun on_TyGoals conjSN phi TG = 
          TG.map (fn (torea, sortscheme) => 
                     (on_ToRea conjSN phi torea, on_SortScheme conjSN phi sortscheme) )  TG
   ...
   (* 14jan02 Previously the following incorrectly assumed that dom R and dom phi were disjoint. *)
      ...
      fun on_RL conjSN phi (REFLATTICE m) =
          let  val onSN = on_SortName' conjSN phi  in
...
*)
            val heading = case tystr_opt of NONE => "sort" 
	                                  | SOME tystr => 
		                              if isemptyCE (CE_of_TyStr tystr) then "sort"
					      else "datasort"
          in
