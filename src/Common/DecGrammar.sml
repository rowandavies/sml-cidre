...
(*  (*[ datasort resolvedDec = 
  ...
  in
	...
	ATEXPexp(i, atexp) => ATEXPexp(f i, map_atexp_info f atexp)
      ...
      | TYPEDexp(i, exp, ty) =>
	  TYPEDexp(f i, map_exp_info f exp, map_ty_info f ty)
      | SORTEDexp(i, exp, tys) =>
	  SORTEDexp(f i, map_exp_info f exp, map (map_ty_info f) tys)
	...
	VALdec(i, tyvars, valbind) => VALdec(f i, tyvars, map_valbind_info f valbind)
      ...
      | DATASORTdec(i,datbind) => 
	  DATASORTdec(f i,map_datbind_info f datbind)
    ...
    and map_valsdesc_info f (VALSDESC(i, id, tys, valsdesc_opt) : valsdesc) : valsdesc =
	  VALSDESC(f i, id, map (map_ty_info f) tys,
    ...
    and map_conbind_info f (CONBIND(i,longid,ty_opt,conbind_opt)): conbind =
      CONBIND(f i, longid, do_opt ty_opt (map_ty_info f),
	...
	ATPATpat(i, atpat) =>
      ...
      | SORTEDpat(i, pat, tys) =>
	  SORTEDpat(f i, map_pat_info f pat, map (map_ty_info f) tys)
	...
	TYVARty(i,tyvar) => TYVARty(f i, tyvar)
      ...
      | INTERty(i,ty,ty') => INTERty(f i, map_ty_info f ty, map_ty_info f ty')
  ...
  fun expansive (harmless_con : longid -> bool) exp =
	...
	| nexp_exp (SORTEDexp(info, exp,  tys)) = nexp_exp exp
	...
	| conexp_exp (SORTEDexp(info, exp, ty)) = conexp_exp exp
	| conexp_exp _ = false
  ...
  local
	...
	TYVARty(_, tv) => tv::res
      ...
      | INTERty(_, ty1, ty2) => fTy ty1 (fTy ty2 res)
  ...
  fun find_topmost_id_in_pat (ATPATpat(_, atpat)): string option = find_topmost_id_in_atpat atpat
    ...
    | find_topmost_id_in_pat (SORTEDpat(_,pat,_)) = find_topmost_id_in_pat pat
  ...
  in
	 ...
	 | SORTEDexp(_, exp, tys) =>
	     let
	       ...
	       val tyTs = map layoutTy tys
               val tyTree = NODE{start="", finish="", indent=0, children=tyTs, childsep=RIGHT ", "}
	     in
	       NODE{start="", finish=" ]*)", indent=0,
		       children=[expT, tyTree],
		       childsep=LEFT " (*[ :> "
		      }
	     end
	...
	of VALdec(_, tyvars, valbind) =>
	 ...
	 | VALsdec(_, valsdesc) =>
	     NODE{start="val ", finish="", indent=4,
		     children = [layoutValsdesc valsdesc],
		     childsep=NOSEP
		     }
	 ...
	 | SORTdec(_, typbind) =>
	     NODE{start="sort ", finish="", indent=5,
		     children=[layoutTypbind typbind],
		     childsep=NOSEP
		    }
    ...
    and layoutValsdesc valsdesc : StringTree =
	let
	    fun treesOfValsdesc (VALSDESC (_, id, tys, valsdesc_opt)) : StringTree list =
	    let
                val tyTs = map layoutTy tys
		val tysT = NODE{start="", finish="", indent=0, children=tyTs, childsep=RIGHT ", "}
		val this = NODE{start="", finish="", indent=0,
				children=[LEAF (Ident.pr_id id), tysT],
				childsep=RIGHT " : " }
	    in
		this :: (case valsdesc_opt
			   of SOME valsdesc => treesOfValsdesc valsdesc
		            | NONE => nil
			)
	    end
	in
	    NODE{start="", finish="", indent=0,
		 children=treesOfValsdesc valsdesc,
		 childsep=LEFT " and "
	        }
	end
    ...
    and layoutConbind conbind : StringTree =
      let
	fun treesOfConbind(CONBIND(_, OP_OPT(longid, withOp), ty_opt, conbind_opt))
	  : StringTree list =
	  let
	    val conT =
	      LEAF((if withOp then "op " else "") ^ Ident.pr_longid longid)
