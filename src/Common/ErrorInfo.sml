...
where type Report = Report.Report
  ...
  struct
    ...
    fun errorIncludesSourceInfo (REFDEC _) = true
      | errorIncludesSourceInfo _ = false
   ...
   (* A lot of the module unification errors carry the same argument types: *)
    ...
    fun report (UNIFICATION (ty1, ty2)) =
	  ...
	  line (Type.pretty_string (StatObject.newTVNames ()) ty ^ 
      ...
      | report (INCOMPATIBLE_TY (ty1, ty2)) =
        let
	    val names = StatObject.newTVNames ()
	    val pr = Type.pretty_string names
	in
	    line "Incompatible types,"
	    // line ("   expecting: " ^ pr ty1)
	    // line ("   found:     " ^ pr ty2)
	end

      | report (INCOMPATIBLE_TYSCH (tysch1, tysch2)) =
        let
(*            val names = StatObject.newTVNames ()
	      fun pr tysch = TypeScheme.pretty_string names tysch
*)
          fun pr tysch = StringTree_to_string(StatObject.TypeScheme.layout tysch)
	in
	    line "Incompatible type schemes,"
	    // line ("   expecting: " ^ pr tysch1)
	    // line ("   found:     " ^ pr tysch2)
	end

      | report (TYGOAL) =
        line "Refinement in signature contains identifiers which are not bound here"

      | report (INTERSECTION_IN_TYPE) =
        line "Intersections are not allowed in types, only in sorts."

      | report (REFDEC errors) =
        Report.flatten (map RefineErrorInfo.report errors)
      ...
      | report QUALIFIED_CON =
	  line ("You may not specify a constructor qualified with a structure.")
      ...
      | report (WHERE_TYPE_MULTIPLE_REFINEMENTS (longtycon, t)) =
	  line ("Type " ^ longtycon_and_tyname_as_string longtycon t 
		^ " has non-trivial refinements, so cannot be instantiated using where.")
          // line "Consider directly writing the instantiated signature instead."
	  ...
	  in case sigmatcherror
		...
		| S_CONFLICTING_DOMCE longtycon =>
		 ...
		 // line ("  " ^ StringTree_to_string(StatObject.TypeScheme.layout sig_sigma) ^ ".")
	       ...
	       | NOSORTENRICHMENT{qualid=(strids, id),str_sigma,str_vce,sig_sigma,sig_vce} =>
		 line ("Error in signature matching:") 
		 // line ("the sort specified in the signature does not enrich")
		 // line ("the sort found in the structure;")
		 // line ("Structure declares:")
		 // line (str_vce ^ " " ^ prStrIds strids  ^ Ident.pr_id id ^ " :")
		 // line ("  " ^ RefObject.pr_SortSchemePRETTY (RefObject.newSVNames ()) str_sigma)
		 // line ("Signature specifies:")
		 // line (sig_vce ^ " " ^ prStrIds strids  ^ Ident.pr_id id ^ " :")
		 // line ("  " ^ RefObject.pr_SortSchemePRETTY (RefObject.newSVNames ())
                                 sig_sigma ^ ".")
		 ...
		 // line("  specified  type: " ^ Type.string ty2)
               ...
               | INCOMPATIBLE_REALISATION longtycon => 
		 line("The sort in the structure refines the wrong type for sortcon: ") 
		 // line("  " ^ TyCon.pr_LongTyCon longtycon)

               | SUBSORT (srt1, srt2) => 
		 line("Subsorting in signature not satisfied by structure.") 
		 // line("  The sort:             " ^ RefObject.pr_SortPRETTY 
	  ...
	  case ei
	    of UNIFICATION _ =>                         "UNIFICATION"
	     ...
	     | REBINDING_IT =>                          "REBINDING_IT"
             | INCOMPATIBLE_TY _ =>                     "INCOMPATIBLE_TY"
             | INCOMPATIBLE_TYSCH _ =>                  "INCOMPATIBLE_TYSCH"
             | TYGOAL =>                                "TYGOAL"
             | INTERSECTION_IN_TYPE =>                  "INTERSECTION_IN_TYPE"
             | REFDEC _ =>                              "REFDEC"
	     | SPECIFYING_TRUE_NIL_ETC _ =>             "SPECIFYING_TRUE_NIL_ETC"
	     | SPECIFYING_IT =>                         "SPECIFYING_IT"
             | QUALIFIED_CON =>                         "QUALIFIED_CON"
	     ...
	     | WHERE_TYPE_MULTIPLE_REFINEMENTS _ =>     "WHERE_TYPE_MULTIPLE_REFINEMENTS"
	     ...
	     | SIGMATCH_ERROR sigmatcherror =>
	      ...
	      in case sigmatcherror
		   of MISSINGSTR _ =>                   "MISSINGSTR"
		    ...
		    | NOSORTENRICHMENT _ =>             "NOSORTENRICHMENT"
		    | EXCNOTEQUAL _ =>                  "EXCNOTEQUAL"
                    | INCOMPATIBLE_REALISATION _ =>     "INCOMPATIBLE_REALISATION"
                    | SUBSORT _ =>                      "SUBSORT"
                    | VARIANCE _ =>                      "VARIANCE"
	      end
	     | CYCLE _ =>                               "CYCLE"
