    ...
    * it is passed around and side-effected. *)
	...
	fun tyvars0 f_b ty : unit = 
	  ...
	  in case #TypeDesc ty 
		...
		| RECTYPE r => RecType.fold (fn (ty, ()) => tyvars0 f_b ty) () 
	  ...
	  | unifyTyVar
	      (ty  as {TypeDesc = TYVAR(tv as ref(NO_TY_LINK{rank=rank_ref as ref r, ...})),
	       ...
	       ty' as {TypeDesc = TYVAR(tv' as ref(NO_TY_LINK{rank=rank_ref' as ref r', ...})),
		       level=level'}) =
	      if TyVar.eq (tv,tv') then 
		if !level = !level' then () else print "Possible BUG: unifyTyVar\n" 
						       (* die "unifyTyVar" *)
	      else 
	...
	fun unifyType(ty,ty') =
	  ...
	  in
            (if !Flags.DEBUG_TYPES then 
              (pr_debug ("unifyType: ", PP.layout_pair layout layout (ty, ty'));
               pr_debug ("           ", PP.layout_list TyVar.layout restricted_tyvars))
             else ();
	...
	(unifyType (tau,tau'); 
         ...
         UnifyOk)
	handle Unify s => UnifyFail before
                          (if !Flags.DEBUG_TYPES then 
                            print ("unifyType: NO  " ^ s ^ "\n")
                           else () )
	     | Rank(tv,tn) => UnifyRankError(tv,tn) before
                          (if !Flags.DEBUG_TYPES then 
                            print "unifyType: NO  (Rank)\n"
                           else () )
      end
	  ...
	  let
	    ...
	    val types = map fresh tyvars
            val _ = (if !Flags.DEBUG_TYPES then 
                            pr_debug ("instance': ", 
                                      PP.layout_pair layout (PP.layout_list Type.layout)
                                                     (sigma, types))
                       else () )
	    val res = instance_with_types (sigma, types)
          in
            (res, types)
            before   (if !Flags.DEBUG_TYPES then 
                            pr_debug ("instance': ", 
                                      PP.layout_pair layout Type.layout (sigma, res))
                       else () )
	...
	fun generalises_Type (sigma, tau') : bool =
	  let
	    ...
	    val fv_tau' = TyVar.unionTyVarSet(Type.tyvars tau', Type.generic_tyvars tau')
	  in
	    ...
	    case Type.restricted_unify 
                   (TyVar.unionTyVarSet(fv_tau',fv_sigma)) (tau,tau')
	...
	fun generalises_TypeScheme (sigma, (betas,tau)) : bool =
            (if !Flags.DEBUG_TYPES then 
               pr_debug ("generalises_TypeScheme: ", 
                         PP.layout_pair layout layout (sigma, (betas, tau)))
             else ();
