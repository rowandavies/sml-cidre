functor Environment(val program_name : unit -> string) :> ENVIRONMENT =
  ...
  local
    ...
    val libPath = OS.FileSys.fullPath "../../lib"  (* Done at (or just after) compile time (hopefully) *)

  in
      fun getEnvVal "SML_LIB" = SOME libPath              (* Cidre hack to redirect to library stubs *)
        | getEnvVal key = (case OS.Process.getEnv(key)
