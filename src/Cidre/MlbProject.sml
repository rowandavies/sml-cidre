functor MlbProject (Env : ENVIRONMENT) :> MLB_PROJECT = 
  ...
  struct
    ...
    type strid=string and sigid=string and funid=string
    ...
    and bdec = SEQbdec of bdec * bdec
             ...
             | STRbdec of (strid * strid) list
             | SIGbdec of (sigid * sigid) list
             | FUNbdec of (funid * funid) list
    ...
    (*   | fold (STRbdec (strid1,strid2)) e bas loc fopen fatbdec fmlb script ann = e  (* bas(bid,bexp,e) *) *)
...
(*	fun error (s : string) = 
  ...
  fun expand mlbfile s = 
    ...
    fun parse_id_pairs mlbfile [] = ([],[])
       | parse_id_pairs mlbfile (id1 :: "=" :: id2 :: ss) =
           if not (is_strid id1 andalso is_strid id2) then  parse_error1 mlbfile ("I expect a structure identifier", ss) else
             let val (rest, ss) = parse_and_id_pairs mlbfile ss  in  ((id1,id2)::rest, ss)
             end
       | parse_id_pairs mlbfile (id :: ss) =
           if not (is_strid id) then  parse_error1 mlbfile ("I expect a structure identifier", ss) else
             let val (rest, ss) = parse_and_id_pairs mlbfile ss  in  ((id,id)::rest, ss)
             end
    and parse_and_id_pairs mlbfile ("and"::ss) = parse_id_pairs mlbfile ss
      | parse_and_id_pairs mlbfile ss = ([],ss)
	...
	and parse_bdec_opt mlbfile ss =
		...
		"local" :: ss => 
	      ...
	      | "structure" :: ss =>
                 (case parse_id_pairs mlbfile ss of ([], ss) => parse_error1 mlbfile ("I expect a structure identifier", ss)
                                          | (id_pairs, ss) => parse_bdec_more mlbfile (MS.STRbdec id_pairs,ss) )

	      | "signature" :: ss =>
                 (case parse_id_pairs mlbfile ss of ([], ss) => parse_error1 mlbfile ("I expect a structure identifier", ss)
                                          | (id_pairs, ss) => parse_bdec_more mlbfile (MS.SIGbdec id_pairs,ss) )
	      | "functor" :: ss =>
                 (case parse_id_pairs mlbfile ss of ([], ss) => parse_error1 mlbfile ("I expect a structure identifier", ss)
                                          | (id_pairs, ss) => parse_bdec_more mlbfile (MS.FUNbdec id_pairs,ss) )
...
(*    val unique = MlbFileSys.unique *)
  ...
  fun pp_prior k [] = ""
    ...
    local 
      ...
      fun check a = check' (list a) (* (sort (list a)) *)
    in
