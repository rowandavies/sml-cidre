...
struct
            ...
            case args of
               [file] =>
                  cm2mlb {defines = !defines,
                          maps = !maps,
                          out = fn s => TextIO.output (TextIO.stdOut,s),
                          sources = file}
             | flag :: args =>
   ...
   fun mlbString file = 
      let val strsref = ref []
          fun out s = strsref := (s :: !strsref)
          val _ = cm2mlb {defines = ["MLton"], maps=[], out=out, sources=file}
      in  
          String.concat (List.rev (!strsref))
      end
