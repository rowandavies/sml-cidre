...
structure TextIO:TEXT_IO = struct(*[ assumesig TEXT_IO ]*)
type elem = unit
type vector = unit
...
type instream = unit
  ...
  val openIn       : string -> instream = fn _ => raise Match
  val closeIn      : instream -> unit = fn _ => raise Match
  val input        : instream -> vector = fn _ => raise Match
  val inputAll     : instream -> vector = fn _ => raise Match

(*not supported by MOSCOW ML either; it raises an exception.
  val inputNoBlock : instream -> vector option = fn _ => raise Match
*)
  val input1       : instream -> elem option = fn _ => raise Match
  val inputN       : instream * int -> vector = fn _ => raise Match
  val inputLine    : instream -> string = fn _ => raise Match
  val endOfStream  : instream -> bool = fn _ => raise Match
  val lookahead    : instream -> elem option = fn _ => raise Match

type cs (* character source state *) = unit
...
type outstream = unit
  ...
  val openOut      : string -> outstream = fn _ => raise Match
  val openAppend   : string -> outstream = fn _ => raise Match
  val closeOut     : outstream -> unit = fn _ => raise Match
  val output       : outstream * vector -> unit = fn _ => raise Match
  val output1      : outstream * elem -> unit = fn _ => raise Match
  val outputSubstr : outstream * Substring.substring -> unit = fn _ => raise Match
  val flushOut     : outstream -> unit = fn _ => raise Match

  val stdOut       : outstream = ()
  val stdErr       : outstream = ()

  val print        : string -> unit = fn _ => raise Match
end; (*signature TEXT_IO*)
...
type of character vectors (strings). = unit
