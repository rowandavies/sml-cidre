...
structure ListPair:LIST_PAIR = struct(*[ assumesig LIST_PAIR ]*)
  val zip : ('a list * 'b list) -> ('a * 'b) list  = fn _ => raise Match
  val unzip : ('a * 'b) list -> ('a list * 'b list)  = fn _ => raise Match
  val map : ('a * 'b -> 'c) -> ('a list * 'b list) -> 'c list  = fn _ => raise Match
  val app : ('a * 'b -> unit) -> ('a list * 'b list) -> unit  = fn _ => raise Match
  val foldl : (('a * 'b * 'c) -> 'c) -> 'c -> ('a list * 'b list) -> 'c  = fn _ => raise Match
  val foldr : (('a * 'b * 'c) -> 'c) -> 'c -> ('a list * 'b list) -> 'c  = fn _ => raise Match
  val all : ('a * 'b -> bool) -> ('a list * 'b list) -> bool  = fn _ => raise Match
  val exists : ('a * 'b -> bool) -> ('a list * 'b list) -> bool  = fn _ => raise Match
