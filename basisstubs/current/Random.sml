...
struct(*[ assumesig RANDOM ]*)
type generator = unit

    val newgenseed : real -> generator = fn _ => raise Match
    val newgen     : unit -> generator = fn _ => raise Match
    val random     : generator -> real = fn _ => raise Match
    val randomlist : int * generator -> real list = fn _ => raise Match
    val range      : int * int -> generator -> int = fn _ => raise Match
    val rangelist  : int * int -> int * generator -> int list = fn _ => raise Match
  end
