...
structure Time:TIME =
struct(*[ assumesig TIME ]*)
type time = unit
    
    exception Time

    val zeroTime : time = ()
    val now      : unit -> time = fn _ => raise Match

    val toSeconds        : time -> int = fn _ => raise Match
    val toMilliseconds   : time -> int = fn _ => raise Match
    val toMicroseconds   : time -> int = fn _ => raise Match
    val fromSeconds      : int -> time = fn _ => raise Match
    val fromMilliseconds : int -> time = fn _ => raise Match
    val fromMicroseconds : int -> time = fn _ => raise Match

    val fromReal : real -> time = fn _ => raise Match
    val toReal   : time -> real = fn _ => raise Match

    val toString   : time -> string	(* rounded to millisecond precision *) = fn _ => raise Match
    val fmt        : int -> time -> string = fn _ => raise Match
    val fromString : string -> time option = fn _ => raise Match
    val scan       : (char, 'a) StringCvt.reader -> (time, 'a) StringCvt.reader = fn _ => raise Match

    val +  : time * time -> time = fn _ => raise Match
    val -  : time * time -> time = fn _ => raise Match
    val <  : time * time -> bool = fn _ => raise Match
    val <= : time * time -> bool = fn _ => raise Match
    val >  : time * time -> bool = fn _ => raise Match
    val >= : time * time -> bool = fn _ => raise Match

    val compare : time * time -> order = fn _ => raise Match
