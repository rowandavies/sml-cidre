...
type dirstream = unit
    ...
    val openDir   : string -> dirstream = fn _ => raise Match
    val readDir   : dirstream -> string = fn _ => raise Match
    val rewindDir : dirstream -> unit = fn _ => raise Match
    val closeDir  : dirstream -> unit = fn _ => raise Match
      
    val chDir     : string -> unit = fn _ => raise Match
    val getDir    : unit -> string = fn _ => raise Match
    val mkDir     : string -> unit = fn _ => raise Match
    val rmDir     : string -> unit = fn _ => raise Match
    val isDir     : string -> bool = fn _ => raise Match
      
    val realPath  : string -> string = fn _ => raise Match
    val fullPath  : string -> string = fn _ => raise Match
    val isLink    : string -> bool = fn _ => raise Match
    val readLink  : string -> string = fn _ => raise Match
      
    val modTime   : string -> Time.time = fn _ => raise Match
    val setTime   : string * Time.time option -> unit = fn _ => raise Match
    val remove    : string -> unit = fn _ => raise Match
    val rename    : {old: string, new: string} -> unit = fn _ => raise Match
    val access    : string * access list -> bool = fn _ => raise Match
    val fileSize  : string -> int = fn _ => raise Match
      
    val tmpName   : unit -> string = fn _ => raise Match
      
type file_id = unit
    val fileId    : string -> file_id = fn _ => raise Match
...
(* These functions operate on the file system.  They raise OS.SysErr
   ...
   [readDir dstr] returns and consumes a entry from the directory
   stream.  Only entries distinct from the parent arc and the current
   arc (that is, .. and . in Unix, DOS, and Windows; see the Path
   structure) will be returned.  When all directory entries have been
   read, so the directory stream is empty, then the empty string "" is
   returned.
