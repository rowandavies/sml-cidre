...
structure Array:ARRAY = struct (*[ assumesig ARRAY ]*)

(*TODO 21/10/1997 11:36. tho.: skal det ikke være eqtype?*)
type 'a array = unit

  val maxLen   : int = 0

  val array    : int * '_a -> '_a array = fn _ => raise Match
  val tabulate : int * (int -> '_a) -> '_a array = fn _ => raise Match
  val fromList : '_a list -> '_a array = fn _ => raise Match

  val length  : 'a array -> int = fn _ => raise Match
  val sub     : 'a array * int -> 'a = fn _ => raise Match
  val update  : 'a array * int * 'a  -> unit = fn _ => raise Match
  val extract : 'a array * int * int option -> 'a Vector.vector = fn _ => raise Match
  ...
  val app     : ('a -> unit) -> 'a array -> unit = fn _ => raise Match
  val foldl   : ('a * 'b -> 'b) -> 'b -> 'a array -> 'b = fn _ => raise Match
  val foldr   : ('a * 'b -> 'b) -> 'b -> 'a array -> 'b = fn _ => raise Match
