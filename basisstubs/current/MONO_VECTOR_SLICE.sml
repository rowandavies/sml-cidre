...
full vec
    ...
    creates a slice representing the entire vector vec. It is
    equivalent to
...
slice (vec, i, sz)
    ...
    creates a slice based on the vector vec starting at index i of the
    vector vec. If sz is NONE, the slice includes all of the elements
    to the end of the vector, i.e., vec[i..|vec|-1]. This raises
    Subscript if i < 0 or |vec| < i. If sz is SOME(j), the slice has
    length j, that is, it corresponds to vec[i..i+j-1]. It raises
    Subscript if i < 0 or j < 0 or |vec| < i + j. Note that, if
    defined, slice returns an empty slice when i = |vec|.
...
base sl
    ...
    representation of the slice. vec is the underlying vector, i is
    the starting index, and n is the length of the slice.

vector sl
    
    generates a vector from the slice sl. Specifically, if vec is the
    resulting vector, we have |vec| = |sl| and, for 0 <= i < |sl|,
    element i of vec is sub (sl, i).

concat l

    is the concatenation of all the vectors in l. This raises Size if
    the sum of all the lengths is greater than the maximum length
...
map f sl
    ...
    These functions generate new vectors by mapping the function f
    from left to right over the argument slice. The more general mapi
    function supplies both the element and the element's index in the
    slice to the function f. The latter expression is equivalent to:
...
foldri f init sl
    ...
    These fold the function f over all the elements of a vector slice,
...
all f sl
    ...
    equivalent to not(exists (not o f) sl)).
