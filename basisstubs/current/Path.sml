...
structure Path:OS_PATH = struct(*[ assumesig OS_PATH ]*)
  ...
  val fromString   : string -> {isAbs : bool, vol : string, arcs : string list} = fn _ => raise Match
  val toString     : {isAbs : bool, vol : string, arcs : string list} -> string = fn _ => raise Match

  val getVolume    : string -> string  = fn _ => raise Match
  val validVolume  : {isAbs : bool, vol : string} -> bool = fn _ => raise Match
  val getParent    : string -> string = fn _ => raise Match

  val isAbsolute   : string -> bool = fn _ => raise Match
  val isRelative   : string -> bool = fn _ => raise Match
  val mkAbsolute   : string * string -> string = fn _ => raise Match
  val mkRelative   : string * string -> string = fn _ => raise Match

  val concat       : string * string -> string = fn _ => raise Match

  val mkCanonical  : string -> string = fn _ => raise Match
  val isCanonical  : string -> bool = fn _ => raise Match

  val splitDirFile : string -> {dir : string, file : string} = fn _ => raise Match
  val joinDirFile  : {dir : string, file : string} -> string = fn _ => raise Match
  val dir          : string -> string = fn _ => raise Match
  val file         : string -> string = fn _ => raise Match

  val splitBaseExt : string -> {base : string, ext : string option} = fn _ => raise Match
  val joinBaseExt  : {base : string, ext: string option} -> string = fn _ => raise Match
  val base         : string -> string     = fn _ => raise Match
  val ext          : string -> string option = fn _ => raise Match
