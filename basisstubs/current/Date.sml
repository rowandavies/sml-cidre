...
type date = unit
    ...
    val year    : date -> int = fn _ => raise Match
    val month   : date -> month = fn _ => raise Match
    val day     : date -> int = fn _ => raise Match
    val hour    : date -> int = fn _ => raise Match
    val minute  : date -> int = fn _ => raise Match
    val second  : date -> int = fn _ => raise Match
    val weekDay : date -> weekday = fn _ => raise Match
    val yearDay : date -> int = fn _ => raise Match
    val isDst   : date -> bool option = fn _ => raise Match
    val offset  : date -> Time.time option = fn _ => raise Match

    val fromTimeLocal : Time.time -> date = fn _ => raise Match
    val fromTimeUniv  : Time.time -> date = fn _ => raise Match
    val toTime        : date -> Time.time = fn _ => raise Match
    val localOffset   : unit -> Time.time = fn _ => raise Match

    val compare    : date * date -> order = fn _ => raise Match
      
    val toString   : date -> string = fn _ => raise Match
    val fmt        : string -> date -> string = fn _ => raise Match
    val fromString : string -> date option = fn _ => raise Match
    val scan       : (char, 'a) StringCvt.reader -> (date, 'a) StringCvt.reader = fn _ => raise Match
