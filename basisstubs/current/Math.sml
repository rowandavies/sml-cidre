...
type real = real
  ...
  val sqrt  : real -> real = fn _ => raise Match
  val sin   : real -> real = fn _ => raise Match
  val cos   : real -> real = fn _ => raise Match
  val tan   : real -> real = fn _ => raise Match
  val atan  : real -> real = fn _ => raise Match
  val asin  : real -> real = fn _ => raise Match
  val acos  : real -> real = fn _ => raise Match
  val atan2 : real * real -> real = fn _ => raise Match
  val exp   : real -> real = fn _ => raise Match
  val pow   : real * real -> real = fn _ => raise Match
  val ln    : real -> real = fn _ => raise Match
  val log10 : real -> real = fn _ => raise Match
  val sinh  : real -> real = fn _ => raise Match
  val cosh  : real -> real = fn _ => raise Match
  val tanh  : real -> real = fn _ => raise Match
end; (*signature MATH*)
